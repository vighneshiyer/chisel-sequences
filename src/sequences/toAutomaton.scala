...
object toAutomaton {
  def apply[S <: Data](prop: Property[S], backendImpl: backend.Backend, initialState: S): Bool = {
    ...
    val mod = backendImpl.compileFSM(info, initialState)
...
private object ToIRConverter {
  def toIR[S <: Data](prop: Property[S], initialState: S): (backend.PropertyInfo[S], Map[String, (S) => Bool]) = new ToIRConverter().toIR(prop, initialState)
}

private class ToIRConverter[S <: Data] private () {
  private val predMap = mutable.LinkedHashMap[(S) => Bool, String]()

  def toIR(prop: Property[S], initialState: S): (backend.PropertyInfo[S], Map[String, (S) => Bool]) = {
    predMap.clear()
  ...
  private def convert[S <: Data](prop: Property[S], initialState: S): backend.Property[S] = prop match {
    case PropSeq(s) => backend.PropSeq(convert(s, initialState))
  }

  private def convert[S <: Data](seq: Sequence[S], state: S): backend.Sequence[S] = seq match {
    case SeqExpr(predicate)     => backend.SeqPred(convert(predicate))
    case SeqStateExpr(predicate: (S => Bool), update) => backend.SeqStatePred(convert(predicate(state)), update)
  ...
  private def convert(e: chisel3.Bool): backend.BooleanExpr = {
    ...
    val name = predMap.getOrElseUpdate(_ => e, f"p${predMap.size}")
